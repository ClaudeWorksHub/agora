...
<!-- ROLE REGISTRY: To add a new role, update ALL THREE locations:
     1. This Role Name Mapping table
     2. Complexity assessment table (below)
     3. Conflict arbitration rules (Independent Assessment section) -->
...
Interactive: {on/off}
Estimated: ~{N + N×max_rounds} opus calls (max, hibernation may reduce)
────────────────────────────
```
...
3. **Check phase** — branch based on `state.phase`:
   ...
   - `"assessment"`: Interrupted during Independent Assessment. Grep the discussion file for `<!-- assessment:{role_name}:start -->` markers to detect which roles completed (fallback for old files: `### {emoji} .+ (的开发计划|的评估|Assessment)`; markers must appear between `## Independent Assessment` and `## Round 1`). Compare with `assessment_completed_roles`; if inconsistent, trust the discussion file. Re-invoke only incomplete roles, then synthesize and proceed to Debate.
...
4. **Consistency check (debate phase only)** — compare `state.current_round` with the max N from all `## Round N` headings found in the discussion file (use Grep). Note: `current_round` tracks the last written `## Round N` heading number. If duplicate `## Round 1` headings are found (legacy format before `## Initial Proposal` rename), output `⚠️ Legacy format detected` and treat the first occurrence as the initial proposal:
...
7. **Resume Debate**: set `round = state.current_round` and enter the Debate loop (agents will re-review the last round's content as a defensive measure). **The first round after resume, all roles MUST be active (no hibernation allowed)**, regardless of their `consecutive_no_objection` or `sleeping` state — to ensure all roles have sufficient context before potentially entering sleep.
...
<!-- assessment:{role_name}:start -->
### {emoji} {role name} Assessment:
{agent's full output}
<!-- assessment:{role_name}:end -->
...
b) Use AskUserQuestion to ask (adapt prompt and option text to match the task description language):
   - Option 1: "Continue" (description: "Proceed to next step, orchestrator responds")
   - Option 2: "I have input" (description: "Enter your opinions or constraints to be included in the discussion")

c) If user selects Option 1 → proceed to Step 2 normally.

d) If user selects Option 2 or provides custom text → treat user input as a **stakeholder directive**:
...
d) **Update the state file** (`proposal-<timestamp>.state.json`) using the Write tool (overwrite with updated JSON). Write order: **always write the discussion file first (step c), then the state file (step d)** — because state can be inferred from the discussion file if needed.
   ...
   - `current_round`: set to `round + 1` (tracks the last written `## Round N` heading number), `consecutive_no_objection`
